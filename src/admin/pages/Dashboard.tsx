...
const Dashboard: React.FC = () => {
  ...
  const [reservations, setReservations] = useState<Reservation[]>([
    {
      ...
      email: "jean.d@lotus.com",
      occasion: "Anniversary",
      table: "Table 12",
      notes: "Window seat preferred. Needs high chair.",
      isPreOrder: true,
      preOrderItems: [
  ...
  const stats = useMemo(
    () => ({
      ...
      cancelled: reservations.filter((res) => res.status === "Cancelled")
        .length,
  ...
  return (
    <div className="relative min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      {/* MAIN CONTENT AREA */}
      <div
        className={`transition-all duration-500 p-8 ${selectedRes ? "mr-[450px] opacity-30 pointer-events-none scale-[0.98]" : ""}`}
      >
        <div className="max-w-7xl mx-auto space-y-10">
          {/* Header & Search */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-4xl font-black text-[#1E293B] italic tracking-tight">
                Lotus Blanc Dashboard
              </h1>
              <p className="text-slate-500 font-medium mt-1">
                Manage your daily restaurant operations
              </p>
            </div>
        ...
        <div className="fixed inset-y-0 right-0 w-[450px] bg-white shadow-2xl z-50 border-l border-slate-100 flex flex-col animate-in slide-in-from-right duration-500">
          ...
          <div className="p-8 border-b border-slate-50 flex justify-between items-center">
            <button
              ...
              className="p-3 bg-slate-100 rounded-2xl text-slate-400 hover:text-slate-800 transition-all"
            >
              <X size={20} />
            </button>
            <div className="flex gap-2">
              <StatusBtn
                active={selectedRes.status === "Arrived"}
                label="Arrive"
                color="emerald"
                onClick={() => updateStatus(selectedRes.id, "Arrived")}
              />
              <StatusBtn
                active={selectedRes.status === "Confirmed"}
                label="Confirm"
                color="blue"
                onClick={() => updateStatus(selectedRes.id, "Confirmed")}
              />
              <StatusBtn
                active={selectedRes.status === "Cancelled"}
                label="Cancel"
                color="red"
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10">
            ...
            <div className="space-y-6">
              ...
              <DetailBox
                icon={<Clock size={20} />}
                label="Arrival Time"
                value={selectedRes.time}
              />
              <DetailBox
                icon={<PartyPopper size={20} />}
                label="Occasion"
                value={selectedRes.occasion}
              />
            </div>
          ...
          <div className="p-8 border-t border-slate-50">
            <button
              ...
              className="w-full py-5 text-red-500 font-black text-xs uppercase tracking-[0.2em] hover:bg-red-50 rounded-[1.5rem] transition-all flex items-center justify-center gap-2"
            >
...
const StatCard = ({ title, value, trend, icon, color }: any) => {
  ...
  return (
    <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
      <div
        className={`absolute -right-6 -top-6 size-24 rounded-full opacity-10 blur-3xl ${color}`}
      />
      <div className="flex justify-between items-start mb-6">
        <div
          className={`p-4 rounded-2xl shadow-sm border border-white/50 ${color} group-hover:scale-110 transition-transform`}
        >
          {React.cloneElement(icon, { size: 22, strokeWidth: 2.5 })}
        </div>
        <div
          className={`flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-black border tracking-tighter shadow-sm ${
            isPositive
      ...
      <div>
        <p className="text-slate-400 text-[11px] font-black uppercase tracking-[0.15em] mb-1.5">
          {title}
        </p>
        <div className="flex items-baseline gap-1.5">
          ...
          {title.toLowerCase().includes("revenue") && (
            <span className="text-slate-300 font-bold text-xs">USD</span>
          )}
        </div>
      </div>
    </div>
...
const ReservationItem = ({
  ...
  <div className="flex items-center justify-between p-6 bg-slate-50/50 rounded-[2.5rem] border border-transparent hover:border-[#034A6C]/10 hover:bg-white transition-all group shadow-sm">
    <div className="flex items-center gap-6">
      <div className="size-14 bg-[#034A6C] text-white rounded-2xl flex items-center justify-center font-black text-xl shadow-lg shadow-blue-50">
        {name[0]}
      </div>
      <div>
        ...
        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
          {time} • {adults + children} Members
        </p>
      </div>
    </div>
    <div className="flex items-center gap-6">
      <span
        className={`text-[9px] font-black uppercase px-4 py-2 rounded-xl border ${
          status === "Arrived"
...
const StatusBtn = ({ label, color, active, onClick }: any) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all ${themes[color]}`}
    >
      {label}
    </button>
  );
...
const ContactRow = ({ icon, label, value }: any) => (
  <div className="flex items-center gap-5 p-2 group cursor-pointer">
    <div className="p-4 bg-slate-50 rounded-2xl text-slate-300 group-hover:text-[#034A6C] transition-colors">
      {icon}
    </div>
    <div>
